...
{#snippet ideaItems(priority: string)}
  {@const items = byPriority(priority)}
  <!-- Column header -->
  <div class="px-3 py-2 border-b text-xs tracking-widest font-bold shrink-0"
       style="border-color: var(--bmo-border); color: {PRIORITY_COLORS[priority]}">
...
<div class="h-full flex flex-col">
  ...
  <PanelCanvas pageId="/ideas">
    <Panel id="ideas:high" label="High Priority" defaultPosition={{ col: 0, row: 0, colSpan: 4, rowSpan: 5 }}>
      {@render ideaItems('high')}
    </Panel>

    <Panel id="ideas:medium" label="Medium Priority" defaultPosition={{ col: 4, row: 0, colSpan: 4, rowSpan: 5 }}>
      {@render ideaItems('medium')}
    </Panel>

    <Panel id="ideas:low" label="Low Priority" defaultPosition={{ col: 8, row: 0, colSpan: 4, rowSpan: 5 }}>
      {@render ideaItems('low')}
    </Panel>

    <Panel id="ideas:add-form" label="Add Idea" defaultPosition={{ col: 0, row: 5, colSpan: 12, rowSpan: 1 }}>
